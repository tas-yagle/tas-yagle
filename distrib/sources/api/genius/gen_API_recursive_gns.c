...
ALL_FOR_GNS *gns_RunGNS(lofig_list *lf, char *celldir, char *libname)
{
  ...
  avt_fprintf(stdout, AVT_MAGENTA "____ Start of GNS sub execution on '%s' ____" AVT_RESET "\n",lf->NAME);
...
/*  lf=rduplofig(lf);
  ...
  avt_fprintf(stdout, AVT_MAGENTA "____ End of GNS sub execution for '%s' ____" AVT_RESET "\n",lf->NAME);
