...
/*   reverse connectors, instances, etc.  
         Pourquoi ???
	 zinaps: parce que... ca marche mieux avec les autres parser/drivers
*/
...
/* write signals */
	for (psig = pfig->LOSIG; psig != NULL; psig = psig->NEXT)
	{
		...
		(void)fprintf(in,"\n");
                ...
                {
			ptrcnet = psig->PRCN;
			if( rcn_getcapa(pfig,psig) >= 0.0 )
				(void)fprintf(in,"Q %g\n",rcn_getcapa(pfig,psig));


			for (ptwire = ptrcnet->PWIRE; ptwire != NULL ; ptwire = ptwire->NEXT)
				(void)fprintf(in,"W %ld,%ld,%s,%g,%g,%g,%g,%g,%g\n",ptwire->NODE1, ptwire->NODE2, "X", ptwire->RESI, ptwire->CAPA, 0.0, 0.0, 0.0, 0.0);
		}
	}
