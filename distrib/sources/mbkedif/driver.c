...
lofig_list *firstlofig;  
{
	...
	EDIF_LIBNAME = avt_gethashvar( MBK_EDIF_LIBNAME );
	if( !EDIF_LIBNAME )
    ...
    if( nb_lib == 0 )
    {
        EDIF_SUFFIXES = avt_gethashvar( MBK_EDIF_SUFFIXES );
        if( !EDIF_SUFFIXES )
        {
...
locon_list *ptcon;
{
	...
	{
        if( mbk_LosigIsVDD( ptcon->SIG ) || mbk_LosigIsVSS( ptcon->SIG ) )
        {
			ptcon = ptcon -> NEXT;
            continue;
        }
...
lofig_list *ptlofig;
{
	...
	while( ptsig != (losig_list *) NULL )
	{
		if( ptsig->TYPE != 'I' || ptlofig->LOINS )
		{
            ...
            if( mbk_LosigIsVDD( ptsig ) || mbk_LosigIsVSS( ptsig ) )
            {
                ptsig = ptsig->NEXT;
                continue;
            }
