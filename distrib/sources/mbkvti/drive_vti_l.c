...
char *tn, *tp;
{
char *s, *avt_gethashvar(); /* what include is needed? stdlib.h but no on pc */
...
struct rcnhns *org;
	...
	if(ptl) {
		fprintf(ptfile,"X %d ",(int)(long)ptl->DATA);
		return;
	}
...
lowire_list *scanwire;
  ...
  lofigchain( ptfig );
	...
	for (ptcon = ptfig->LOCON; ptcon; ptcon = ptcon->NEXT) {
		...
		if (mbk_LosigIsVDD(ptcon->SIG))
			sig_vdd = con_id;
		else if (mbk_LosigIsVSS(ptcon->SIG))
			sig_vss = con_id;
	...
	for (ptsig = ptfig->LOSIG; ptsig; ptsig = ptsig->NEXT) {
		if (ptsig->TYPE != EXTERNAL) {
			...
			ptsig->INDEX = con_id;
			if (mbk_LosigIsVDD(ptsig))
				sig_vdd = con_id;
			else if (mbk_LosigIsVSS(ptsig))
				sig_vss = con_id;
			ptrcnet=ptsig->PRCN;
			if(ptrcnet)
				if(!gettabnode(ptrcnet))
	...
	for (ptcon = ptfig->LOCON; ptcon; ptcon = ptcon->NEXT) {
		if( (scannum = ptcon->PNODE) ) {
			for( ; scannum ; scannum=scannum->NEXT ) {
				...
				ptnode->USER=addptype(ptnode->USER,HNSRCN_X,(void*)(long)idx_rcn);
	...
	for (pttrs = ptfig->LOTRS; pttrs; pttrs = pttrs->NEXT) {
		...
		(void)fprintf(ptfile, "%c ", MLO_IS_TRANSN(pttrs->TYPE) ? tn : tp);
		(void)fprintf(ptfile, "* ");
		...
		(void)fprintf(ptfile, "%ld ", MLO_IS_TRANSN(pttrs->TYPE) ? sig_vss : sig_vdd);
		(void)fprintf(ptfile, ";\n");
	}
	...
	for (ptsig = ptfig->LOSIG; ptsig; ptsig = ptsig->NEXT) {
		...
		if ((ptsig->TYPE == INTERNAL && ptsig->NAMECHAIN != NULL) || ptrcnet) {
			...
			(void)fprintf(ptfile, "%s ", busname(getsigname(ptsig)));
			(void)fprintf(ptfile, ";\n");
			if (ptsig->PRCN && rcn_getcapa( ptfig,ptsig ) && strcmp(OUT_LO, "hns")) {
				(void)fprintf(ptfile, "C 0 * * ");           /* type */
				(void)fprintf(ptfile, "%.3e ", rcn_getcapa( ptfig,ptsig ) ); /* val  */
