...
/*
 * $Log: parse_vti_p.c,v $
 * Revision 1.2  2002/07/02 16:22:23  gregoire
 * Portage 64 bits + qques warning.
 *
 * Revision 1.1.1.1  2001/09/04 14:49:24  sources
 *
 *
 ...
 *
 */
#ident "$Id: parse_vti_p.c,v 1.2 2002/07/02 16:22:23 gregoire Exp $"
...
phcon_list  *ptcon; \
 \
   for (ptcon = figure->PHCON; ptcon; ptcon = ptcon->NEXT) \
      if (((int)(long)(getptype(ptcon->USER, (long)VTIPARSER)->DATA)) == refnum) \
         break; \
 \
   ...
   while (1) {
      ...
      switch (line[0]) {
         ...
         case 'D' : /* D num num name name num num num */
            ...
            addpoint(l1, l2);
            if (mode != 'P') {
               if (isdigit((int)*t4)) /* for vti v8 compatibility */
