...
static struct begex *beh_addgexex(unsigned int oper, unsigned short type)
...
static struct begex *beh_addterm (unsigned int oper)
  {
  ...
  if (bank >= bank_nbr)
    {
    for (i=bank_nbr ; (unsigned)i<=bank ; i++)
      {
...
void *mad_fregex(struct begex *ptgex, char mode)
...
static struct begex *flatgex(struct begex *gexpnt)
...
static struct begex *rmvconst(struct begex *gexpnt)
...
static struct begex *beh_abl2gex(befig_list *ptr_befig, chain_list *expr)
...
void beh_makgex (befig_list *pt_befig)
