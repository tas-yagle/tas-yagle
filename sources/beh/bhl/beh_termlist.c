...
/* ###--------------------------------------------------------------### */
/* function	: beh_termlist						*/
/* description	: find the list of terminals for an expression. The	*/
/*		  value returned by the function is the list of		*/
/*		  terminals. Each terminal is represented by its name	*/
/*		  in the list. A given terminal may appear several	*/
/*		  times in the list. Constante values are note added	*/
/*		  to the list.						*/
/* called func.	: beh_termlist, namealloc, addchain, append		*/
/* ###--------------------------------------------------------------### */

struct chain *beh_termlist (pt_exp)
  ...
  if (pt_exp != NULL)
	...
	/* ###------------------------------------------------------### */
      ...
      if (((int) ((struct chain *) pt_exp->DATA)->DATA) == STABLE)
        ...
        while (pt_opr != NULL)
          {
          tmp_chain = beh_termlist (pt_opr->DATA);
