...
#ident  "$Id: mbk_ph.c,v 1.3 2008/03/27 15:16:16 fabrice Exp $"

/*
 * $Log: mbk_ph.c,v $
 * Revision 1.3  2008/03/27 15:16:16  fabrice
 * . possibilite compilation avec derniere version de gcc
 * . tcl8.5.1
 * . champ CATEG de certaines APIs
 *
 * Revision 1.2  2002/06/25 08:44:59  fabrice
 * *** empty log message ***
 *
 * Revision 1.1.1.1  2001/09/04 14:46:48  sources
 *
 *
...
phseg_list *addphseg(phfig_list *ptfig, char layer, long width, long x1, long y1, long x2, long y2, char *nodename)
{
   ...
   if ((layer >= CALU1) && (layer <= CALU9))
      if (nodename == NULL || *nodename == (char)0 || !strcmp(nodename, "*")) {
         (void)fflush(stdout);
         (void)fprintf(stderr, "*** mbk error ***\nillegal addphseg\n");
         (void)fprintf(stderr, "Connection layer (%ld) without a name\n",
                               (long)layer);
         (void)fprintf(stderr, "x1=%ld y1=%ld x2=%ld y2=%ld\n", x1, y1, x2, y2);
         EXIT(1);
      }
