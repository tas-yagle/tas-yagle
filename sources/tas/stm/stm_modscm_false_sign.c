...
/*    Fichier : stm_modscm_false_sign.c                                     */
/*                                                                          */
...
/****************************************************************************/
#ifdef NEWSIGN
float stm_modscm_false_signature (falseparams *params, char *s)
{
  int i, j, index;
  float sign=0;
  ...
  if(params->LI){
      ...
      for (i = 0; i < mbk_long_round(params->LI[STM_NB_I_LINKS]); i++) {
  ...
  if(params->LN){
      for (i=0; i<STM_NB_LINK_N_PARAMS; i++)
        sign+=params->LN[i]*(i+17);
  }

  /* dual part */
  for (i=0; i<STM_NB_FALSE_PARAMS; i++)
    sign+=params->FP[i]*(i+23);

  return sign;
}
#else
void stm_modscm_false_signature (falseparams *params, char *s)
{
    int i;
    int index;
    float precision = 0.01;

    sprintf (s + strlen (s), "scmf_");
    ...
    /* dual part */
    sprintf (s + strlen (s), "f_");
    sprintf (s + strlen (s), "%d_", mbk_float_approx_interval(params->FP[STM_PCONF0_F], precision));
    sprintf (s + strlen (s), "%d_", mbk_float_approx_interval(params->FP[STM_PCONF1_F], precision));
    sprintf (s + strlen (s), "%.5g_", params->FP[STM_RTOT_F]);
    sprintf (s + strlen (s), "%.5g_", params->FP[STM_K_F]);
    sprintf (s + strlen (s), "%.5g_", params->FP[STM_VDDMAX_F]);
    sprintf (s + strlen (s), "%d_", mbk_float_approx_interval(params->FP[STM_DRC_F], precision));
    sprintf (s + strlen (s), "%.5g", params->FP[STM_VF_INPUT_F]);
}
#endif
/****************************************************************************/

void stm_modscm_false_change_params (falseparams *params)
{
    int i, j;
    int index;
    float precision = 0.01;
    ...
    /* links 1 to n - 1 */
    if(params->LI){
//        params->LI[STM_NB_I_LINKS] = mbk_rounddouble(params->LI[STM_NB_I_LINKS], STM_NEWSIGN_ROUND_PRECISION);
        for (i = 0; i < params->LI[STM_NB_I_LINKS]; i++) {
            index = 1 + STM_NB_LINK_I_PARAMS * i;
            for (j=0; j<STM_NB_LINK_I_PARAMS; j++)
    ...
    /* dual part */
    for (i=0; i<STM_NB_FALSE_PARAMS; i++)
      params->FP[i] = mbk_rounddouble(params->FP[i], STM_NEWSIGN_ROUND_PRECISION);
#else
    /* link 0 */
    if(params->L0){
    ...
    /* dual part */
    params->FP[STM_PCONF0_F] = mbk_float_approx_middle(params->FP[STM_PCONF0_F], precision);
    params->FP[STM_PCONF1_F] = mbk_float_approx_middle(params->FP[STM_PCONF1_F], precision);
    params->FP[STM_DRC_F] = mbk_float_approx_middle(params->FP[STM_DRC_F], precision);
#endif
}

#ifdef NEWSIGN
int stm_modscm_false_same_params (falseparams *params0, falseparams *params1)
{
  ...
  return memcmp(params0->FP, params1->FP, STM_NB_FALSE_PARAMS * sizeof (float))==0;
}
#endif
